...
#define HT_AVAIL_ERROR_MAX 10
	...
	if (on) {
		...
		if (!SBSDIO_CLKAV(clkctl, bus->alp_only)) {
			...
			/* Fallback: try FORCE_HT to bypass firmware PMU dependency */
			DHD_ERROR(("%s: Trying SBSDIO_FORCE_HT bypass\n", __FUNCTION__));
			bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR,
				(SBSDIO_HT_AVAIL_REQ | SBSDIO_FORCE_HT), &err);
			OSL_DELAY(50000); /* 50ms for PLL to stabilize */
			clkctl = bcmsdh_cfg_read(sdh, SDIO_FUNC_1,
				SBSDIO_FUNC1_CHIPCLKCSR, &err);
			DHD_ERROR(("%s: After FORCE_HT: clkctl 0x%02x\n",
				__FUNCTION__, clkctl));
			if (!SBSDIO_CLKAV(clkctl, bus->alp_only)) {
				DHD_ERROR(("%s: FORCE_HT also failed: clkctl 0x%02x\n",
					__FUNCTION__, clkctl));
				/* Poll CHIPCLKCSR for 500ms to see if firmware changes it.
				 * Uses only CMD52 (safe even if AXI bus is stalled).
				 * If clkctl changes, firmware IS running; if stuck, firmware not running.
				 */
				{
					int _pi;
					uint8 _prev = clkctl;
					DHD_ERROR(("%s: polling CSR for 500ms...\n", __FUNCTION__));
					for (_pi = 0; _pi < 50; _pi++) {
						uint8 _c;
						int _e = 0;
						OSL_DELAY(10000); /* 10ms */
						_c = bcmsdh_cfg_read(sdh, SDIO_FUNC_1,
							SBSDIO_FUNC1_CHIPCLKCSR, &_e);
						if (_c != _prev) {
							DHD_ERROR(("%s: CSR changed at %dms: 0x%02x\n",
								__FUNCTION__, (_pi+1)*10, _c));
							_prev = _c;
						}
					}
					DHD_ERROR(("%s: final CSR after 500ms: 0x%02x\n",
						__FUNCTION__, _prev));
				}
				/* Dump PMU registers to understand why HT is unavailable */
				{
					uint _pmucap = si_corereg(bus->sih, SI_CC_IDX,
						OFFSETOF(chipcregs_t, pmucapabilities), 0, 0);
					uint _pmust  = si_corereg(bus->sih, SI_CC_IDX,
						OFFSETOF(chipcregs_t, pmustatus), 0, 0);
					uint _resst  = si_corereg(bus->sih, SI_CC_IDX,
						OFFSETOF(chipcregs_t, res_state), 0, 0);
					uint _minres = si_corereg(bus->sih, SI_CC_IDX,
						OFFSETOF(chipcregs_t, min_res_mask), 0, 0);
					uint _maxres = si_corereg(bus->sih, SI_CC_IDX,
						OFFSETOF(chipcregs_t, max_res_mask), 0, 0);
					uint _reqmsk = si_corereg(bus->sih, SI_CC_IDX,
						OFFSETOF(chipcregs_t, res_req_mask), 0, 0);
					uint _xtalfq = si_corereg(bus->sih, SI_CC_IDX,
						OFFSETOF(chipcregs_t, pmu_xtalfreq), 0, 0);
					uint _clkst  = si_corereg(bus->sih, SI_CC_IDX,
						OFFSETOF(chipcregs_t, clk_ctl_st), 0, 0);
					DHD_ERROR(("PMU: cap=0x%x stat=0x%x res_state=0x%x\n",
						_pmucap, _pmust, _resst));
					DHD_ERROR(("PMU: min_res=0x%x max_res=0x%x req_mask=0x%x\n",
						_minres, _maxres, _reqmsk));
					DHD_ERROR(("PMU: xtalfreq=0x%x clk_ctl_st=0x%x\n",
						_xtalfq, _clkst));
				}
				return BCME_ERROR;
			}
			DHD_ERROR(("%s: FORCE_HT succeeded\n", __FUNCTION__));
		}
#endif /* BCMSDIOLITE */
...
dhdsdio_download_state(dhd_bus_t *bus, bool enter)
{
	...
	if (enter) {
		...
		if (!foundcr4) {
			...
			/* Keep D11/802.11 MAC core fully in reset (disabled) during firmware
			 * download and startup. si_core_disable holds resetctrl=1 while
			 * enabling the clock so the core is accessible but cannot issue
			 * any AXI transactions that could stall the ARM CM3 before it
			 * reaches PMU initialization.
			 */
			if (si_setcore(bus->sih, D11_CORE_ID, 0)) {
				si_core_disable(bus->sih, 0);
				DHD_ERROR(("D11 core disabled (fully in reset)\n"));
			}
		...
		if (!si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
			...
			if (si_setcore(bus->sih, ARM7S_CORE_ID, 0)) {
				DHD_ERROR(("ARM core found: ARM7S\n"));
			} else if (si_setcore(bus->sih, ARMCM3_CORE_ID, 0)) {
				DHD_ERROR(("ARM core found: ARMCM3\n"));
			} else {
				DHD_ERROR(("%s: Failed to find ARM core!\n", __FUNCTION__));
				bcmerror = BCME_ERROR;
				goto fail;
			}
			/* Read SRAM[0..7] to verify firmware content */
			{
				uint32 sram0[2] = {0, 0};
				dhdsdio_membytes(bus, FALSE, 0, (uint8 *)sram0, 8);
				DHD_ERROR(("SRAM[0]=0x%08x SRAM[4]=0x%08x\n",
					sram0[0], sram0[1]));
			}
			/* Dump all enumerated cores to diagnose missing D11 */
			{
#define SI_MAXCORES_DIAG 32
				uint _clist[SI_MAXCORES_DIAG];
				uint _nc = si_corelist(bus->sih, _clist);
				uint _ci;
				DHD_ERROR(("SI cores (%u):", _nc));
				for (_ci = 0; _ci < _nc && _ci < SI_MAXCORES_DIAG; _ci++)
					DHD_ERROR((" 0x%03x", _clist[_ci]));
				DHD_ERROR(("\n"));
			}
		} else {
		...
		/* Diagnostic: check ARM CM3 AI wrapper state right after si_core_reset */
		{
			uint _ioctrl = si_wrapperreg(bus->sih, 0x408, 0, 0);
			uint _resetctrl = si_wrapperreg(bus->sih, 0x800, 0, 0);
			DHD_ERROR(("ARM CM3 wrapper: ioctrl=0x%x resetctrl=0x%x isup=%d\n",
				_ioctrl, _resetctrl, si_iscoreup(bus->sih)));

			/* If ARM CM3 is still in reset (si_core_reset loop timed out),
			 * force-clear resetctrl and re-enable clock via AI wrapper registers.
			 */
			if (!si_iscoreup(bus->sih)) {
				DHD_ERROR(("%s: ARM CM3 stuck in reset, force-releasing\n",
					__FUNCTION__));
				/* Clear resetctrl: AIRC_RESET = 1 → write 0 */
				si_wrapperreg(bus->sih, 0x800, ~0u, 0);
				OSL_DELAY(1000);
				/* Set ioctrl = SICF_CLOCK_EN only (no FGC) */
				si_wrapperreg(bus->sih, 0x408, ~0u, SICF_CLOCK_EN);
				OSL_DELAY(1000);
				DHD_ERROR(("%s: After force: ioctrl=0x%x resetctrl=0x%x isup=%d\n",
					__FUNCTION__,
					si_wrapperreg(bus->sih, 0x408, 0, 0),
					si_wrapperreg(bus->sih, 0x800, 0, 0),
					si_iscoreup(bus->sih)));
			}
		}

		/* BCM43436b0 (26MHz xtal) requires firmware to configure the PMU PLL
		 * before HT becomes available. Stay in ALP-only mode so dhd_bus_init
		 * succeeds immediately; the firmware will enable HT once running.
		 */
		/* bus->alp_only = FALSE; -- deferred until firmware enables HT PLL */

		/* Diagnostic: read CHIPCLKCSR and SLEEPCSR right after ARM release */
		{
			int _err = 0;
			uint8 _csr = bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
				SBSDIO_FUNC1_CHIPCLKCSR, &_err);
			uint8 _sleep = bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
				SBSDIO_FUNC1_SLEEPCSR, &_err);
			DHD_ERROR(("ARM released: CHIPCLKCSR=0x%02x SLEEPCSR=0x%02x\n",
				_csr, _sleep));
		}
