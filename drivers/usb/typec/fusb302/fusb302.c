...
#else
	chip->connector_fwnode = fusb302_fwnode_get(&client->dev);
	if (IS_ERR(chip->connector_fwnode)) {
		ret = PTR_ERR(chip->connector_fwnode);
		dev_err(&client->dev,
			"failed to get connector node %d\n", ret);
		return ret;
	}

	chip->role_sw = fwnode_usb_role_switch_get(chip->connector_fwnode);
